...
MCScatterView {
	...
	*new {|parent, bounds, data, specX, specY|
	...
	highlightItemRel_{|val|
		this.highlightItem = (val * (markers.size-1)).asInteger;
	}
	...
	highlightRangeRel{|start, end|
		...
		startIndex = (start * (markers.size-1)).asInteger;
		endIndex = (end * (markers.size-1)).asInteger;
	...
	data_{|data|
		markers = data.collect {|item|
			[specX.unmap(item[0]), specY.unmap(item[1])];
		};
		highlightItem = min(highlightItem, (markers.size-1));
	}
	symbolSize_{|point|
	...
	initPlot {
		...
		this.data_(data);
		// IS this for highlighted points?
			...
			.drawFunc_({|w|
				...
				GUI.pen.use{
					...
					if (possibleMethods.includes(drawMethod), {
						...
						if (markers.notNil) {
							GUI.pen.moveTo(
								((markers[0][0]  * width) + (pad/2))
								 @
								 (((1-markers[0][1]) * height) + (pad/2))
							   	  +
					...
					if(isHighlight) {
						highlightItem.do{|item|
							cross.value((Rect(
								(markers[item][0]  * width)
								 -
								 (highlightSize.x/2) + (pad/2),
								((1-markers[item][1]) * height)
								 -
								 (highlightSize.y/2)
								  +
								  (pad/2),
